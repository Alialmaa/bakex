...
export default function ReportsPage({ user, data, totals }: any) {
    ...
    <Layout user={user} lang={lang} setLang={setLang}>
      <div style={{display:'flex',flexDirection:'column',gap:12}}>
        ...
        <div className="card">
          ...
          <div className="trow thead" style={{gridTemplateColumns:'22px 1.5fr 80px 80px 80px 80px 80px'}}>
            <span>#</span><span>{lang==='ar'?'المنتج':'Product'}</span><span>{lang==='ar'?'كوست الحبة':'Unit Cost'}</span><span>{lang==='ar'?'سعر البيع':'Sell Price'}</span><span>{lang==='ar'?'المبيعات':'Sales'}</span><span>{lang==='ar'?'الإيراد':'Revenue'}</span><span>{lang==='ar'?'الربح':'Profit'}</span>
          </div>
          ...
          {sorted.map((d: any, i: number) => {
            ...
            return (
              <div key={d.name} className="trow" style={{gridTemplateColumns:'22px 1.5fr 80px 80px 80px 80px 80px'}}>
                ...
                <span style={{fontSize:12}}>{d.unitCost.toFixed(2)} {cur}</span>
                <span style={{fontSize:12}}>{d.sellPrice.toFixed(2)} {cur}</span>
                <span style={{fontSize:12}}>{d.qty}</span>
                <span style={{fontSize:12}}>{d.revenue.toFixed(0)} {cur}</span>
                <span style={{fontSize:12,fontWeight:500,color:pc}}>{d.profit>=0?'+':''}{d.profit.toFixed(0)} {cur}</span>
              </div>
            )
          })}
        </div>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
          {[{title:t.reports.topProducts,icon:'🏆',list:[...sorted].slice(0,3),isTop:true},{title:t.reports.botProducts,icon:'📉',list:[...sorted].reverse().slice(0,3),isTop:false}].map(({title,icon,list,isTop}) => (
...
export const getServerSideProps: GetServerSideProps = async ({ req }) => {
  ...
  const [{ data: sales }, { data: recipes }, { data: stock }] = await Promise.all([
