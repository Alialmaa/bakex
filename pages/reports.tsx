...
export default function ReportsPage({ user, data, totals, prodSummary }: any) {
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 8 }}>
              {prodSummary.map((p: any, i: number) => (
                <div key={i} style={{ background: '#f5f5f3', padding: '10px 12px', borderRadius: 8 }}>
                  ...
                  <div style={{ fontSize: 16, fontWeight: 500 }}>{p.total} <span style={{ fontSize: 11, color: '#888' }}>{p.output_unit}</span></div>
                  <div style={{ fontSize: 10, color: '#3B6D11' }}>{lang === 'ar' ? 'كوست:' : 'Cost:'} {p.totalCost?.toFixed(0)} {cur}</div>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="card">
            ...
            <div style={{ display: 'flex', gap: 5 }}>
              {(['profit', 'margin', 'cost'] as const).map(s => (
                <button key={s} onClick={() => setSort(s)} className={`tag ${s === 'profit' ? 'tag-green' : s === 'margin' ? 'tag-yellow' : 'tag-red'}`} style={{ cursor: 'pointer', border: sort === s ? '1.5px solid #1D9E75' : '1.5px solid transparent', padding: '3px 10px' }}>
                  {s === 'profit' ? (lang === 'ar' ? 'الأعلى ربحاً' : 'Most Profitable') : s === 'margin' ? (lang === 'ar' ? 'الهامش' : 'By Margin') : (lang === 'ar' ? 'الأغلى كوست' : 'Highest Cost')}
                </button>
              ))}
            </div>
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: '22px 1.5fr 80px 80px 80px 80px 80px', gap: 8, padding: '6px 0 8px', borderBottom: '0.5px solid #d4d4d4', fontSize: 11, color: '#888', fontWeight: 500 }}>
            <span>#</span><span>{lang === 'ar' ? 'المنتج' : 'Product'}</span><span>{lang === 'ar' ? 'الكوست' : 'Cost'}</span><span>{lang === 'ar' ? 'السعر' : 'Price'}</span><span>{lang === 'ar' ? 'المباع' : 'Sold'}</span><span>{lang === 'ar' ? 'الإيراد' : 'Revenue'}</span><span>{lang === 'ar' ? 'الربح' : 'Profit'}</span>
          </div>
          ...
          {sorted.map((d: any, i: number) => {
            ...
            return (
              <div key={d.name} style={{ display: 'grid', gridTemplateColumns: '22px 1.5fr 80px 80px 80px 80px 80px', gap: 8, alignItems: 'center', padding: '8px 0', borderBottom: '0.5px solid #e5e5e5', fontSize: 13 }}>
                ...
                <span style={{ fontSize: 12 }}>{d.unitCost.toFixed(2)}</span>
                <span style={{ fontSize: 12 }}>{d.sellPrice.toFixed(2)}</span>
...
export const getServerSideProps: GetServerSideProps = async ({ req }) => {
  ...
  // Production summary - group production_log by recipe
  ...
  const prodSummary = Object.values(prodMap)
  ...
  return { props: { user, data, totals, prodSummary } }
}
