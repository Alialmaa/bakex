...
export default function StockPage({ user, initialStock }: any) {
  ...
  const addMaterial = async () => {
    ...
    setSaving(true)
    await fetch('/api/stock', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ ...form, qty: parseFloat(form.qty), min_qty: parseFloat(form.min_qty)||0, price_per_unit: parseFloat(form.price_per_unit)||0 }) })
    await refresh()
    ...
    <Layout user={user} lang={lang} setLang={setLang}>
      <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:8 }}>
          ...
          <div style={{background:stock.filter(m=>m.qty<=0).length?'#FCEBEB':'#f5f5f3',borderRadius:8,padding:'12px 14px'}}>
            <div className="metric-label" style={{color:stock.filter(m=>m.qty<=0).length?'#A32D2D':''}}>{t.stock.empty}</div>
            <div className="metric-value" style={{color:stock.filter(m=>m.qty<=0).length?'#A32D2D':''}}>{stock.filter(m=>m.qty<=0).length}</div>
          </div>
        </div>

        {/* Table */}
        <div className="card" style={{padding:0,overflow:'hidden'}}>
          <div className="trow thead" style={{gridTemplateColumns:'1.5fr 100px 70px 90px 90px 80px 70px',padding:'8px 16px'}}>
            <span>{t.stock.material}</span><span>{t.stock.qty}</span><span>{t.stock.unit}</span><span>{t.stock.min}</span><span>{t.stock.price} ({t.currency})</span><span>{lang==='ar'?'قيمة المخزن':'Value'}</span><span>{t.stock.status}</span>
          </div>
          <div style={{padding:'0 16px'}}>
            {stock.map(item => {
              const st = getStatus(item)
              return (
                <div key={item.id} className="trow" style={{gridTemplateColumns:'1.5fr 100px 70px 90px 90px 80px 70px'}}>
                  <span style={{fontWeight:500}}>{item.name}</span>
                  <div style={{display:'flex',alignItems:'center',gap:5}}>
                    <button onClick={() => updateQty(item.id, Math.max(0, item.qty-1))} style={{width:24,height:24,borderRadius:6,border:'0.5px solid #d4d4d4',background:'#f5f5f3',cursor:'pointer',fontSize:15,display:'flex',alignItems:'center',justifyContent:'center'}}>−</button>
                    <span style={{minWidth:30,textAlign:'center',fontWeight:500}}>{item.qty}</span>
                    <button onClick={() => updateQty(item.id, item.qty+1)} style={{width:24,height:24,borderRadius:6,border:'0.5px solid #d4d4d4',background:'#f5f5f3',cursor:'pointer',fontSize:15,display:'flex',alignItems:'center',justifyContent:'center'}}>+</button>
                  </div>
